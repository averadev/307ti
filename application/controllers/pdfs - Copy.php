...
class Pdfs extends CI_Controller {
	...
	public function reportMaintanance(){
		...
		foreach ($contracts as $key) {
			...
			$body .= '<tr class="fuenteP"><td class="first">INVOICE DATE:</td><td>'. date("d/m/Y", strtotime($key->Date)) .'</td><td>AMOUNT DUE:</td><td>'. number_format((float)$key->Amount, 2, '.', '').'</td>';

			$body .= '</tr>';
			$body .= '<tr class="fuenteP"><td class="first">DUE DATE:</td><td>'. 'JANUARY 15, 2017' .'</td><td colspan="2"></td><td colspan="2">PLEASE RETURN THIS STUB WITH YOUR PAYMENT</td></tr>';
